...
export function esbuildServerSideJs({
	...
	externals.forEach((EXTERNAL) => {
		debug && console.log(`EXTERNAL:${toStr(EXTERNAL)}`);
		//ignoreFiles.push(`${srcDir}/${EXTERNAL}.es`); // Nope we need to build the externals too
		COMMAND_ARGS.push(`--external:${EXTERNAL}`);
	});
	debug && console.log(`COMMAND_ARGS:${toStr(COMMAND_ARGS)}`);
	const COMMAND = COMMAND_ARGS.join(' ');
	debug && console.log(`COMMAND:${toStr(COMMAND)}`);
	let stdout = '';
